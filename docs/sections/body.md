...
- **Auto-refreshing token** — Setlist refreshes the Withings OAuth2 token when it expires.

No writes — purely read-only. No entries in the vault; everything comes live from Withings.

## Data source

- **Withings OAuth2** credentials at `$SETLIST_INTEGRATIONS_DIR/withings/credentials.json` and refresh token at `.../withings/token.json`. See [README § Withings setup](../../README.md#withings-optional).
