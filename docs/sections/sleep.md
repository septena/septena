...
No writes — Septena never sends anything back to Oura or Apple. If neither integration is configured, the section shows empty state.

## Data source

- **Oura** — personal access token at `$SEPTENA_INTEGRATIONS_DIR/oura/token.txt`.
- **Apple Health** — Health Auto Export posts to `$SEPTENA_INTEGRATIONS_DIR/health_auto_export/latest.json`.
